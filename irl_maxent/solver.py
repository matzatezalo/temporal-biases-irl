...
def value_iteration_naive(p, reward, discount, eps=1e-3):
    """
    Basic value-iteration algorithm to solve the given MDP.
    ...
    while delta > eps:  # iterate until convergence
        ...
        # max_q = np.zeros((n_actions, n_states, delays))
        # compute state-action values (note: we actually have Q[a, s, d] here)
        q = np.zeros((n_actions, n_states, delays))
        for a in range(n_actions):
            for d in range(delays):  # d = delay
                discount_factor = 1 / (1 + discount * d)  # compute discount factor based on delay
                q[a, :, d] = discount_factor * p[a] @ v

        m = np.max(q, axis=1)
        m1 = np.max(m)
        indices = np.where(q == m1)
        d1 = indices[2][0]
        # compute state values
        v = reward / (1 + discount * d1) + m1
...
def value_iteration_sophisticated(p, reward, discount, eps=1e-3):
    """
    Basic value-iteration algorithm to solve the given MDP.
...
def optimal_policy_naive(world, reward, discount, eps=1e-3):
    """
    Compute the optimal policy using value-iteration
    ...
    See also:
        ...
        - `optimal_policy_from_value`
    """
    value = value_iteration_naive(world.p_transition, reward, discount, eps)
    return optimal_policy_from_value(world, value)


def optimal_policy_soph(world, reward, discount, eps=1e-3):
    """
    Compute the optimal policy using value-iteration
    ...
    See also:
        ...
        - `optimal_policy_from_value`
    """
    value = value_iteration_sophisticated(world.p_transition, reward, discount, eps)
    return optimal_policy_from_value(world, value)
